...
const showingInfo = document.querySelector('.showing-info');
  ...
  showingInfo.textContent = `Showing ${startIndex + 1} - ${endIndex} of ${items.length} (Page ${page} of ${totalPages})`;
}
...
for (let i = 1; i <= totalPages; i++) {
  ...
  html += '<span class="page-number-gap">&nbsp;</span>';
}
...
// Function to update row highlighting for all checkboxes
...
editIcons.forEach(icon => {
  icon.addEventListener('click', e => {
    ...
    if (!existingSaveBtn) {
      ...
      saveBtn.innerHTML = '<i class="fa-solid fa-check-to-slot"></i>';
